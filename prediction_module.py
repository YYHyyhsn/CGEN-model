...
from model.data import collate_pool
from model.cgcnn import CrystalGraphConvNet
# Set the standard output to UTF-8 encoding
...
# Create a simulated args object
class Args:
...
# Initialize args with the configuration file
args = Args(config)
...
if os.path.isfile(args.modelpath):
    ...
    # Loading the training parameters from the checkpoint
    model_args = Args(model_checkpoint['args'])
...
def main():
    ...
    # Build the model - Use the parameters at the time of training
...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
        ...
        # Calculate the accuracy rate and record the loss
        if model_args.task == 'regression':
    ...
    if test:
        ...
        # In the case of a regression task, calculate and output RMSE and R²
        if model_args.task == 'regression':
            # Make sure all data is floating-point numbers
            ...
            print(f' {star_label} RMSE: {rmse:.4f}')
            print(f' {star_label} R²: {r2:.4f}')
